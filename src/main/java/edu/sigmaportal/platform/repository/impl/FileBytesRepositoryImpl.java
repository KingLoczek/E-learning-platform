...
public class FileBytesRepositoryImpl implements FileBytesRepository {
    ...
    public FileModel saveWithContents(FileModel file, InputStream contents, long length) {
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement("INSERT INTO \"FileStorage\" (file_path, file_type_id, bytes, owner_id) VALUES (?, ?, ?, ?) RETURNING file_id");
            ...
            stmt.setInt(4, file.ownerId());

            try (ResultSet results = stmt.executeQuery()) {
                if (!results.next()) throw new RepositoryException("File not inserted");

                int fileId = results.getInt(1);
                return new FileModel(fileId, file.filePath(), file.fileTypeId(), file.ownerId(), file.bytes());
            }
    ...
    public Optional<FileModel> findByIdWithContents(Integer id) {
        try {
            Connection conn = source.getConnection();
            PreparedStatement stmt = conn.prepareStatement("SELECT file_id, file_path, file_type_id, owner_id, bytes FROM \"FileStorage\" WHERE file_id = ?");
            stmt.setInt(1, id);
            ...
            try (ResultSet results = stmt.executeQuery()) {
                ...
                InputStream stream = results.getBinaryStream(5);

                if (results.next()) throw new RepositoryException("Multiple rows found, where only one was expected");

                return Optional.of(new FileModel(fileId, filePath, fileTypeId, ownerId, stream));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
